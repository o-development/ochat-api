...
const fileUploadHandler: IHttpHandler = (app) => {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (
        !req.query.chat_uri ||
        typeof req.query.chat_uri !== "string" ||
        !req.query.mime_type ||
        typeof req.query.mime_type !== "string" ||
        !req.query.file_name ||
        typeof req.query.file_name !== "string"
      ) {
        throw new HttpError(
      ...
      const chatHandler = await externalChatHanderFactory(chatUri, undefined, {
        fetcher: fetch,
      });
      const fileUri = await chatHandler.saveFile(
        req.file.buffer,
        mimeType,
        fileName
      );
