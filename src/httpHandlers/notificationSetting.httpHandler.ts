...
} from "../notification/notificationSubscriptionApi";
...
import {
  ...
  unMuteChat,
  getChatMuteSetting,
} from "../notification/notificationMuteSettingApi";
...
const notificationSettingHandler: IHttpHandler = (app) => {
  ...
  app.post("/notification/subscription/:clientId", async (req, res) => {
    const notificationWebSubscriptionRequest = toINotificationSubscription(
      req.body
    );
    const authSession = getLoggedInAuthSession(req);
    if (typeof req.params.clientId !== "string") {
