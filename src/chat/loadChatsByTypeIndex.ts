...
export async function getTypeIndexUris(options: {
  fetcher?: IFetcher;
  webId: string;
}): Promise<string[]> {
  ...
  const typeIndexUris: string[] = profileNode
    .out(privateTypeIndex)
    .values.concat(profileNode.out(publicTypeIndex).values)
    .filter((val): boolean => val != undefined) as string[];

  return typeIndexUris;
}

export default async function loadChatsByTypeIndex(options: {
  fetcher?: IFetcher;
  webId: string;
}): Promise<void> {
  ...
  if (typeIndexUris.length === 0) {
    throw new HttpError(
      "Your profile does not have any type indexes. You must link chats manually.",
      404
    );
  }
  // Get chat uris from the type index
  const chatUris: string[] = (
    await Promise.all(
      typeIndexUris.map(
        async (typeIndexUri): Promise<string[]> => {
          const dataset = await fetchClownfaceDataset(
            typeIndexUri,
            options.fetcher
          );
          const qualifiedQuads = dataset.dataset.match(
            null,
            forClass,
            LongChat
          );
